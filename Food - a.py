...
def food_a_callback(ch, method, properties, body):
    try:
        ...
        print(f"Received Food-A temperature: {temperature}°C")
        if len(food_a_temperature_deque) >= 5:
            temp_changes = [food_a_temperature_deque[i] - food_a_temperature_deque[i - 1] for i in range(-1, -5, -1)]
            if any(temp_change > FOOD_TEMP_CHANGE_THRESHOLD for temp_change in temp_changes):
                show_food_a_alert(timestamp)
            if len(food_a_temperature_deque) % 5 == 0:
...
def main():
    ...
    for queue in queues:
        ...
        if queue == 'Food-A':
            channel.basic_consume(queue=queue, on_message_callback=food_a_callback)
        else:
